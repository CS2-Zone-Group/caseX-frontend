...
export default function SkinDetailsModal({ isOpen, onClose, skin }: SkinDetailsModalProps) {
  ...
  const translations = {
    en: {
      ...
      tradeProtectionText: 'This item is protected by our secure trading system',
      buyNow: 'Buy Now',
    ...
    ru: {
      ...
      tradeProtectionText: 'Этот предмет защищен нашей безопасной торговой системой',
    ...
    uz: {
      ...
      tradeProtectionText: 'Bu buyum bizning xavfsiz savdo tizimimiz bilan himoyalangan',
  ...
  const getFloatCondition = (floatValue: number) => {
    ...
    if (floatValue < 0.38) return language === 'uz' ? 'Sinab ko\'rilgan' : language === 'ru' ? 'После полевых испытаний' : 'Field-Tested';
    if (floatValue < 0.45) return language === 'uz' ? 'Yaxshi eskirgan' : language === 'ru' ? 'Поношенное в боях' : 'Well-Worn';
    return language === 'uz' ? 'Jang eskirgan' : language === 'ru' ? 'Закаленное в боях' : 'Battle-Scarred';
  };
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="space-y-6">
                  <div className="text-center">
                    ...
                    <div className="w-64 h-48 mx-auto bg-gray-100 dark:bg-gray-800 rounded-lg overflow-hidden flex items-center justify-center">
                      {skin.imageUrl ? (
                        ...
                        <div className="text-gray-400 dark:text-gray-500">{t.noImageAvailable}</div>
                      )}
                    </div>
                    ...
                    <div className="flex space-x-3 mt-4">
                      <button 
                        ...
                        onClick={() => {
                          const steamUrl = `https://steamcommunity.com/market/listings/730/${encodeURIComponent(skin.marketHashName || skin.name)}`;
                  ...
                  <div className="bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-500/30 rounded-lg p-4">
                    <div className="flex items-center space-x-2 mb-2">
                      ...
                      <span className="text-blue-600 dark:text-blue-400 font-medium">{t.tradeProtection}</span>
                    </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-green-500 font-bold text-xl">${typeof skin.price === 'number' ? skin.price.toFixed(2) : typeof skin.price === 'string' ? parseFloat(skin.price).toFixed(2) : '0.00'}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 gap-4">
                    <div>
                      <span className="text-gray-600 dark:text-gray-400 block mb-1">{t.tradeLock}:</span>
                      <span className="text-gray-900 dark:text-white">{t.noTradeLock}</span>
                    </div>

                    <div>
                      <span className="text-gray-600 dark:text-gray-400 block mb-1">{t.itemLocation}:</span>
                      <span className="text-gray-900 dark:text-white">{t.steam}</span>
                    </div>

                    <div>
                      <span className="text-gray-600 dark:text-gray-400 block mb-1">{t.itemType}:</span>
                      <span className="text-gray-900 dark:text-white">{skin.weaponType}</span>
                    </div>

                    <div>
                      <span className="text-gray-600 dark:text-gray-400 block mb-1">{t.exterior}:</span>
                      <span className="text-gray-900 dark:text-white">{skin.exterior}</span>
                    </div>

                    <div>
                      <span className="text-gray-600 dark:text-gray-400 block mb-1">{t.rarity}:</span>
                    ...
                    {skin.float !== undefined && (
                      <div>
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center justify-between">
                            <span className="text-gray-900 dark:text-white font-mono">{typeof skin.float === 'number' ? skin.float.toFixed(4) : typeof skin.float === 'string' ? parseFloat(skin.float).toFixed(4) : '0.0000'}</span>
                            <span className="text-gray-600 dark:text-gray-400 text-sm">
                              {typeof skin.float === 'number' ? getFloatCondition(skin.float) : typeof skin.float === 'string' ? getFloatCondition(parseFloat(skin.float)) : 'Unknown'}
                            </span>
                          </div>
                          <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                            ...
                            >
                              <div 
                                ...
                                style={{ left: `${typeof skin.float === 'number' ? skin.float * 100 : typeof skin.float === 'string' ? parseFloat(skin.float) * 100 : 0}%`, transform: 'translateX(-50%)' }}
                              />
                            </div>
                          </div>
                    ...
                    {skin.collection && (
                      <div>
                        <span className="text-gray-600 dark:text-gray-400 block mb-1">{t.collection}:</span>
                        <span className="text-gray-900 dark:text-white">{skin.collection}</span>
                      </div>
                    )}
                    ...
                    {skin.description && (
                      <div>
                        ...
                        <span className="text-gray-900 dark:text-white">{skin.description}</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex space-x-2">
                      <button className="px-3 py-1 text-sm bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white rounded">7D</button>
                      <button className="px-3 py-1 text-sm bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white rounded">1M</button>
                      <button className="px-3 py-1 text-sm bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white rounded">6M</button>
