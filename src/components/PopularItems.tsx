...
const floatDict: Record<string, { uz: string; ru: string; en: string }> = {
  ...
  'Minimal Wear': {
    uz: 'Biroz ishlatilgan',
    ru: 'Немного поношенное',
    en: 'Minimal Wear'
  },
  'Field-Tested': {
    uz: 'Maydonda sinalgan',
...
const PopularItems = () => {
  ...
  const tFloat = (floatKey: string) => {
    return floatDict[floatKey]?.[language] || floatKey;
  }
      ...
      <section className="py-20 px-4 bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
        <div className="container mx-auto">
          <h2 className="text-3xl md:text-5xl font-black uppercase tracking-tighter mb-12 text-center text-transparent bg-clip-text bg-gradient-to-b from-gray-900 to-gray-600 dark:from-white dark:to-gray-500">
            {language === 'uz' ? 'MASHHUR MAHSULOTLAR' : language === 'ru' ? 'ПОПУЛЯРНЫЕ ТОВАРЫ' : 'POPULAR ITEMS'}
          </h2>
              ...
              >
                ...
                <div className='flex justify-between items-end border-t border-gray-100 dark:border-gray-800 pt-5 mt-2'>
                  <div className='flex flex-col gap-1'>
                    <p className='text-[10px] font-bold tracking-widest text-gray-400 uppercase'>{tFloat(item.float)}</p>
                    <div className='flex flex-col'>
                      <span className='text-xs text-gray-500 dark:text-gray-400 font-medium'>Start from:</span>
