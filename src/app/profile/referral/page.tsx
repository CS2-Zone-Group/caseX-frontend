...
export default function ReferralPage() {
  ...
  // Shart tushuntirmasi
  const conditionText = stats?.completionCondition === 'transaction'
    ? `${t('conditionBuySell')}${stats?.minTransactionAmount ? ` ($${stats.minTransactionAmount}+)` : ''}`
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-5">
                <h3 className="text-sm font-semibold text-blue-400 mb-2">{t('howItWorks')}</h3>
                <ol className="text-sm text-gray-300 space-y-1.5 list-decimal list-inside">
                  <li>{t('step1')}</li>
                  <li>{t('step2', { count: required })}</li>
                  <li><span className="text-yellow-400">{t('condition')}:</span> {conditionText}</li>
                  <li>{t('step3')}</li>
                </ol>
              </div>
                ...
                <div className="mb-4">
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className="text-gray-500">
                      {(stats?.claimedCount || 0) > 0 ? t('rewardReceived') : t('noRewardYet')}
                    </span>
                  </div>
